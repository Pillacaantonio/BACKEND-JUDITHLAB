...
namespace SGE_BACKEND_JUDITH_LAB.Services
{
    public class UsuarioService
    {
        ...
        public async Task<BaseResponse<Usuarios>> UsuarioGetById(int usua_icod_usuario)
        {
            ...
            try
            {
                using (var conexion = _conexion.ObtenerConnexion())
                {
                    ...
                    response.Data = await conexion.QueryFirstAsync<Usuarios>("USP_USUARIO_GET_BY_ID", parametros, commandType: CommandType.StoredProcedure);
        ...
        public async Task<BaseResponse<IEnumerable<Usuarios>>> UsuarioList()
        {
            var response = new BaseResponse<IEnumerable<Usuarios>>();
            try
            {
                using (var conexion = _conexion.ObtenerConnexion())
                {
                    response.Data = await conexion.QueryAsync<Usuarios>("USP_USUARIO_LISTAR", commandType: CommandType.StoredProcedure);
                }
            }
            catch (Exception ex)
            {
